...
func debugContextSize(ctx context.Context) {
	fmt.Fprintln(ginkgo.GinkgoWriter, "DEBUG\t"+debugContextSizeSub(ctx))
}

func debugContextSizeSub(ctx context.Context) string {
	if ctx == nil {
		return ""
	}

	return debugContextSizeSub(GetParent(ctx)) + "\t" + fmt.Sprintf("%d", GetTestCount(ctx))
}
